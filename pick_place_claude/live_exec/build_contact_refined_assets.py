...
CONTACT_ASSET_DIR = BENCHMARK_ROOT / "contact_assets_claude"
...
def build_refined_tube(*, force: bool) -> Path:
    ...
    # Option A: keep the working body+grip_sleeve grip pair, drop only the
    # 25mm grip_collar (which stuck out 7mm beyond the visible cap and was the
    # main source of the rendered "fingers floating off tube" gap). The 18.5mm
    # sleeve is only ~0.75mm beyond the visible cap (sub-pixel) and is what
    # actually generates the squeeze force in the working baseline.
...
def build_refined_robot(*, force: bool) -> Path:
    ...
    # Option A: keep contact_pad + v_groove + x_stop (these all interact with the
    # 18.5mm grip_sleeve and provide the friction grip story). Drop only the
    # contact_shelf, which existed to hook UNDER the dropped 25mm grip_collar
    # and serves no purpose without that collar.
    for side_name, side_sign in (("left_finger", -1.0), ("right_finger", 1.0)):
        ...
        bevel_y = side_sign * (GRIPPER_PAD_CONTACT_LOCAL_Y_M - 0.0005)
        _add_collision_cube(
            stage,
            Gf,
            UsdGeom,
