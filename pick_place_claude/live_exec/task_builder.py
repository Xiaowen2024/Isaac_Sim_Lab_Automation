...
# The live reference videos show a very shallow top grasp, and that is also the
# most physically defensible way to stop relying on an inward fake finger
# pocket. Keep the contact point only a few millimeters below the tube top so
# the fingers pinch the cap/shoulder region first.
GRASP_TOOL_CLEARANCE_M = -0.004
...
# The old lateral pickup bias was tuned around the nonphysical inward pocket.
# Reset to a centered cap grasp while the finger contact geometry is moved back
# onto the real G1 pad wall.
...
def tube_top_grasp_pose(tube_root_pose_w: PoseWxyz) -> PoseWxyz:
    return _pose_from_local_point(tube_root_pose_w, TUBE_TOP_LOCAL_M, TOP_GRASP_QUAT_WXYZ)
