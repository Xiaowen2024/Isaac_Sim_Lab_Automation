...
from wetlab_benchmark.pick_place_claude.live_exec.protocol import (
...
from wetlab_benchmark.pick_place_claude.frames_to_mp4 import encode_png_sequence_to_mp4
...
class LiveIsaacExecutor(Node):
    ...
    def _check_arm_goal_completion(self, sim_time: float) -> None:
        ...
        elapsed = max(sim_time - goal.start_sim_time, 0.0)
        final_positions = self._reorder_joint_positions(goal.joint_names, goal.points[-1].positions)
    ...
    def _sample_arm_positions(self, goal: TrajectoryGoalState, elapsed: float, time_scale: float) -> torch.Tensor:
        ...
        if elapsed <= first_time:
            ...
            first = self._reorder_joint_positions(goal.joint_names, points[0].positions)
            return goal.initial_positions * (1.0 - alpha) + first * alpha

        for index in range(len(points) - 1):
            t0 = _point_time(points[index]) * time_scale
            t1 = _point_time(points[index + 1]) * time_scale
            if elapsed <= t1:
                p0 = self._reorder_joint_positions(goal.joint_names, points[index].positions)
                p1 = self._reorder_joint_positions(goal.joint_names, points[index + 1].positions)
                alpha = 0.0 if t1 <= t0 else (elapsed - t0) / (t1 - t0)
                return p0 * (1.0 - alpha) + p1 * alpha
        return self._reorder_joint_positions(goal.joint_names, points[-1].positions)
    ...
    def _check_physical_grasp(self, sim_time: float) -> None:
        ...
        if cmd.contact_confirmed_at is not None:
            ...
            if bilateral_contact:
                ...
                centered_contact = (
                    float(grasp_extra["tube_to_pad_mid_m"]) <= args_cli.grasp_center_tol_m
                    and float(grasp_extra["tube_up_axis_z"]) >= self.upright_cos_min
                )
