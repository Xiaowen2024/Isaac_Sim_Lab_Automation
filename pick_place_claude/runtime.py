...
GRIPPER_JOINT_STIFFNESS = 6000.0
GRIPPER_JOINT_DAMPING = 260.0
GRIPPER_JOINT_EFFORT_LIMIT = 60.0
IK_DLS_LAMBDA = 0.002
CONTACT_PHYSICS_DT = 0.0025
# Keep the original 3mm contact halo for now. The system was tuned around this
# value; shrinking it independently of geometry caused the gripper to miss the
# tube during approach. Re-tighten only after confirming geometry works.
...
FINGER_RUBBER_PHYSICS_MATERIAL = sim_utils.RigidBodyMaterialCfg(
    ...
    friction_combine_mode="max",
    restitution_combine_mode="min",
    compliant_contact_stiffness=4500.0,
    compliant_contact_damping=120.0,
)
ASSET_PROFILE_IMPORTED = "imported"
ASSET_PROFILE_CONTACT_REFINED = "contact_refined"
CONTACT_ASSET_DIR = Path(__file__).resolve().parents[1] / "contact_assets_claude"
    ...
    if asset_profile == ASSET_PROFILE_CONTACT_REFINED:
        ...
        if missing_assets:
            raise FileNotFoundError(
                ...
                "wetlab_benchmark/pick_place_claude/live_exec/build_contact_refined_assets.py --force` first. "
