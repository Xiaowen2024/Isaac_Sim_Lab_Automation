...
CONTACT_ASSET_DIR = BENCHMARK_ROOT / "contact_assets"
...
def _add_visual_cube(stage, Gf, UsdGeom, path: str, *, center, size, color, rotate_xyz_deg: tuple[float, float, float] | None = None) -> None:
    cube = UsdGeom.Cube.Define(stage, path)
    cube.CreateSizeAttr(1.0)
...
def build_refined_robot(*, force: bool) -> Path:
    ...
    # pad and continue moving the active contact band outward so the visible
    # pad inserts, not the finger frame, are the first thing the tube meets.
    # Retain only a very small upper shelf bias for collar breakout support.
    finger_shell_color = (0.07, 0.07, 0.08)
    pad_visual_color = (0.78, 0.80, 0.82)
    for side_name, side_sign in (("left_finger", -1.0), ("right_finger", 1.0)):
        base = f"/UF_ROBOT/{side_name}"
        finger_visual = stage.GetPrimAtPath(f"{base}/visuals")
        if finger_visual and finger_visual.IsValid():
            UsdGeom.Imageable(finger_visual).MakeInvisible()
        main_inner_face_abs_y = GRIPPER_PAD_CONTACT_LOCAL_Y_M - 0.00020
        main_pad_size_y = 0.0038
        main_y = side_sign * (main_inner_face_abs_y + 0.5 * main_pad_size_y)
        bevel_inner_face_abs_y = (GRIPPER_PAD_CONTACT_LOCAL_Y_M - 0.00010) - 0.00002
        bevel_size_y = 0.0009
        bevel_y = side_sign * (bevel_inner_face_abs_y + 0.5 * bevel_size_y)
        # Keep only a mild inward shelf bias so the replay render stays closer
        # to the visible finger wall while the collar still sees a static
        # support feature near the fingertip.
        shelf_inner_face_abs_y = (GRIPPER_PAD_CONTACT_LOCAL_Y_M - 0.00012) - 0.00030
        shelf_size_y = 0.0030
        shelf_y = side_sign * (shelf_inner_face_abs_y + 0.5 * shelf_size_y)
        shelf_z = min(GRIPPER_PAD_CONTACT_LOCAL_Z_M + 0.0045, 0.0545)
        _add_collision_cube(
            stage,
            Gf,
            UsdGeom,
            ...
            size=(0.028, main_pad_size_y, 0.015),
            material=material,
        )
        visual_shell_y = main_y + side_sign * 0.0054
        visual_shell_z = GRIPPER_PAD_CONTACT_LOCAL_Z_M + 0.0140
        _add_visual_cube(
            stage,
            Gf,
            UsdGeom,
            f"{base}/visual_finger_tip",
            center=(0.0, visual_shell_y, visual_shell_z),
            size=(0.021, 0.0032, 0.0042),
            color=finger_shell_color,
        )
        visual_pad_y = main_y + side_sign * 0.0021
        visual_pad_z = GRIPPER_PAD_CONTACT_LOCAL_Z_M + 0.0072
        _add_visual_cube(
            stage,
            Gf,
            UsdGeom,
            f"{base}/visual_contact_pad",
            center=(0.0, visual_pad_y, visual_pad_z),
            size=(0.018, 0.0026, 0.012),
            color=pad_visual_color,
        )
        _add_collision_cube(
            stage,
            Gf,
            ...
            size=(0.018, shelf_size_y, 0.0070),
            material=material,
        )
        _add_collision_cube(
            stage,
            ...
            size=(0.018, bevel_size_y, 0.0105),
        ...
        x_stop_z = GRIPPER_PAD_CONTACT_LOCAL_Z_M + 0.0015
        _add_collision_cube(
            stage,
            Gf,
            UsdGeom,
            ...
            center=(-0.012, main_y, x_stop_z),
            size=(0.0032, 0.0030, 0.018),
            material=material,
        )
        _add_collision_cube(
            stage,
