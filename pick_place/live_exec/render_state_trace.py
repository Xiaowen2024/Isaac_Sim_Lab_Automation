...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Optional cap on replayed frames for debugging; 0 means all frames.",
)
parser.add_argument(
    "--start_frame",
    type=int,
    default=0,
    help="Optional starting frame index for debugging; defaults to 0.",
)
parser.add_argument(
    "--opaque_tube_visual",
    action=argparse.BooleanOptionalAction,
    default=False,
    help="Override the tube visual with an opaque off-white material during replay render.",
)
parser.add_argument(
    "--tube_visual_color",
    type=float,
    nargs=3,
    default=(0.92, 0.94, 0.96),
    metavar=("R", "G", "B"),
    help="Replay-only diffuse RGB for the tube visual when --opaque_tube_visual is enabled.",
)
parser.add_argument(
    "--high_contrast_grasp_visual",
    action=argparse.BooleanOptionalAction,
    default=False,
    help="Darken gripper-side visuals during replay render so tube/gripper contact is easier to read.",
)
...
from wetlab_benchmark.pick_place.live_exec.protocol import camera_from_preset
from wetlab_benchmark.pick_place.live_exec.task_builder import LiveTask, PoseWxyz, build_live_task
...
def _bind_visual_material_recursive(
    prim_path: str,
    *,
    material_name: str,
    ...
    metallic: float = 0.0,
    include_path_tokens: tuple[str, ...] | None = None,
) -> None:
    import omni.usd
    ...
    normalized_tokens = tuple(token.lower() for token in include_path_tokens or ())
    for descendant in _iter_descendants(prim):
        descendant_path = descendant.GetPath().pathString
        if normalized_tokens and not any(token in descendant_path.lower() for token in normalized_tokens):
            continue
        if descendant.IsA(UsdGeom.Gprim):
            gprim_paths.append(descendant_path)
    if not gprim_paths and prim.IsA(UsdGeom.Gprim):
        gprim_paths.append(prim.GetPath().pathString)
    for gprim_path in gprim_paths:
        sim_utils.bind_visual_material(gprim_path, material_path)
...
def main() -> None:
    ...
    print(f"[render_state_trace] stage=task_built wall_s={time.time()-start_wall:.2f}", flush=True)
    ...
    sim.reset()
    scene.reset()
    print(f"[render_state_trace] stage=scene_reset wall_s={time.time()-start_wall:.2f}", flush=True)
    ...
    print(f"[render_state_trace] stage=runtime_initialized wall_s={time.time()-start_wall:.2f}", flush=True)
    ...
    scene.update(0.0)
    print(f"[render_state_trace] stage=scene_seeded wall_s={time.time()-start_wall:.2f}", flush=True)
    if args_cli.opaque_tube_visual:
        _bind_visual_material_recursive(
            "/World/envs/env_0/Tube",
            material_name="ReplayTubeOpaque",
            color=tuple(float(v) for v in args_cli.tube_visual_color),
            roughness=0.72,
            metallic=0.0,
        )
        print(f"[render_state_trace] stage=tube_material_bound wall_s={time.time()-start_wall:.2f}", flush=True)
    if args_cli.high_contrast_grasp_visual:
        _bind_visual_material_recursive(
            "/World/envs/env_0/Robot",
            material_name="ReplayGripperContrast",
            color=(0.10, 0.10, 0.12),
            roughness=0.58,
            metallic=0.0,
            include_path_tokens=("gripper", "finger", "knuckle"),
        )
        _bind_visual_material_recursive(
            "/World/envs/env_0/Holder",
            material_name="ReplayHolderContrast",
        ...
        _bind_visual_material_recursive(
            ...
            material_name="ReplayVortexerContrast",
            color=(0.88, 0.89, 0.91),
            roughness=0.82,
            metallic=0.0,
        )
        sim.render()
        print(f"[render_state_trace] stage=contrast_material_bound wall_s={time.time()-start_wall:.2f}", flush=True)
    ...
    print(f"[render_state_trace] stage=samples_loaded count={len(samples)} wall_s={time.time()-start_wall:.2f}", flush=True)
    if args_cli.start_frame > 0:
        samples = samples[args_cli.start_frame :]
    if args_cli.max_frames > 0:
        samples = samples[: args_cli.max_frames]
    for frame_index, sample in enumerate(samples):
        ...
        if frame_index in (0, 1, 2) or ((frame_index + 1) % 50 == 0):
            print(
                f"[render_state_trace] stage=frame_written frame={frame_index} wall_s={time.time()-start_wall:.2f}",
                flush=True,
            )
