...
TOP_GRASP_PREGRASP_RETRIES = 5
TOP_GRASP_PREGRASP_MIN_DURATION_S = 20.0
...
def _trajectory_duration_s(trajectory) -> float:
    joint_trajectory = trajectory if isinstance(trajectory, JointTrajectory) else _serialize_moveit_trajectory(trajectory)
    if not joint_trajectory.points:
        return 0.0
    return _duration_to_seconds(joint_trajectory.points[-1].time_from_start)
...
class LiveMoveItRunner(Node):
    ...
    def _do_leg(
        ...
        *,
        ...
        preplanned_breakout = self._load_cached_trajectory(f"{leg.label}:breakout")
        if preplanned_breakout is None:
            preplanned_breakout = self._plan_to_pose(
                leg.breakout,
                label=f"{leg.label}:breakout_preplan",
        ...
        self._send_arm_trajectory(preplanned_breakout, label=f"{leg.label}:breakout")
        final_lift = self._plan_to_pose(
            leg.lift,
            label=f"{leg.label}:lift_preplan",
            plan_params=self.plan_params_lin,
        )
        self._send_arm_trajectory(final_lift, label=f"{leg.label}:lift")
    ...
    def _plan_to_pose(self, goal, *, label: str, plan_params: PlanRequestParameters | None):
        ...
        pipeline_name = getattr(plan_params, "planning_pipeline", "default")
        self.get_logger().info(f"{label}: planning begin pipeline={pipeline_name}")
        prefer_long_top_pregrasp = (
            label.endswith(":pregrasp")
            and abs(float(os.environ.get("WETLAB_TOP_GRASP_TOOL_YAW_DEG", "0.0"))) > 1.0e-6
        )
        attempts = TOP_GRASP_PREGRASP_RETRIES if prefer_long_top_pregrasp else 1
        ...
        for attempt in range(1, attempts + 1):
            ...
            if not plan_result:
                self.get_logger().warning(f"{label}: planning attempt {attempt}/{attempts} failed")
                continue
            trajectory = plan_result.trajectory
            duration_s = _trajectory_duration_s(trajectory)
            self.get_logger().info(f"{label}: planning attempt {attempt}/{attempts} duration_s={duration_s:.2f}")
            if duration_s > best_duration_s:
                best_duration_s = duration_s
                best_trajectory = trajectory
            if not prefer_long_top_pregrasp or duration_s >= TOP_GRASP_PREGRASP_MIN_DURATION_S:
                self.get_logger().info(f"{label}: planning done")
                return trajectory
        if best_trajectory is None:
            raise RuntimeError(f"{label}: planning failed")
        if prefer_long_top_pregrasp:
            self.get_logger().warning(
                f"{label}: using best available long-branch candidate duration_s={best_duration_s:.2f} "
                f"(target >= {TOP_GRASP_PREGRASP_MIN_DURATION_S:.2f})"
            )
        self.get_logger().info(f"{label}: planning done")
